...
void dx_set_current_limit(dynamixel_t *dx, uint8_t id, uint16_t limit) {
    uint8_t buf[4];
    buf[0] = DX_ADDR_CURRENT_LIMIT;
    buf[1] = 0;
    buf[2] = limit & 0x00ff;
    buf[3] = (limit >> 8) & 0x00ff;

    dx_send_packet(dx, id, DX_INS_WRITE, buf, 4);
}
...
void dx_set_goal_position(dynamixel_t *dx, uint8_t id, uint16_t position, uint8_t sync) {
    uint8_t buf[6];
    buf[0] = DX_ADDR_GOAL_POSITION;
    buf[1] = 0;
    buf[2] = position & 0x00ff;
    buf[3] = (position >> 8) & 0x00ff;
    buf[4] = (position >> 16) & 0x00ff;
    buf[5] = (position >> 24) & 0x00ff;
